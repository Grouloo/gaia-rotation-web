...
function semiver(a: any, b: any, bool: boolean) {
	a = a.split('.')
	b = b.split('.')
	return (
		fn(a[0], b[0]) ||
...
function determine_protocol(endpoint: string) {
...
async function process_endpoint(app_reference: string, token: string) {
	const headers = {}
	if (token) {
		// @ts-expect-error
...
async function upload(file_data, root, upload_id, upload_fn = upload_files) {
	...
	return await Promise.all(
		await upload_fn(root, files, void 0, upload_id).then(async (response) => {
			if (response.error) {
	...
	async function client2(app_reference, options = {}) {
		return new Promise(async (res) => {
			...
			async function config_success(_config) {
				...
				api_map = map_names_to_ids(
					(_config == null ? void 0 : _config.dependencies) || [],
				)
			...
			function submit(endpoint, data, event_data, trigger_id = null) {
									...
									let callback = async function (_data) {
										try {
											...
											if (type === 'update' && status2 && !complete) {
												...
												console.error(
													'Unexpected error',
													status2 == null ? void 0 : status2.message,
												)
												fire_event({
													type: 'status',
													stage: 'error',
											...
											if (
												(status2 == null ? void 0 : status2.stage) ===
													'complete' ||
												(status2 == null ? void 0 : status2.stage) === 'error'
											) {
				...
				function off(eventType, listener) {
					...
					listeners =
						listeners == null ? void 0 : listeners.filter((l) => l !== listener)
					narrowed_listener_map[eventType] = listeners
					return { on, off, cancel, destroy }
				}
			...
			function open_stream() {
				...
				event_stream.onmessage = async function (event) {
					...
					if (!event_id) {
						await Promise.all(
							Object.keys(event_callbacks).map((event_id2) =>
			...
			async function component_server(component_id, fn_name, data) {
				...
				if (
					(_a = component == null ? void 0 : component.props) == null
						? void 0
						: _a.root_url
				) {
	...
	async function handle_blob2(endpoint, data, api_info, token) {
		...
		return Promise.all(
			blob_refs.map(async ({ path, blob, type }) => {
				if (blob) {
					const file_url = (await upload_files2(endpoint, [blob], token))
						.files[0]
					return {
						path,
						file_url,
		...
		await Promise.all(
			param.map(async (v, i) => {
				...
				const array_refs = await walk_and_store_blobs(
					param[i],
					root
						? ((_a = api_info == null ? void 0 : api_info.parameters[i]) == null
								? void 0
								: _a.component) || void 0
						: type,
					new_path,
...
function skip_queue(id, config) {
	...
	return (
		!(((_b =
			(_a = config == null ? void 0 : config.dependencies) == null
				? void 0
				: _a[id]) == null
			? void 0
			: _b.queue) === null
			? config.enable_queue
			: (_d =
					(_c = config == null ? void 0 : config.dependencies) == null
						? void 0
						: _c[id]) == null
			? void 0
			: _d.queue) || false
...
async function resolve_config(fetch_implementation, endpoint, token) {
	...
	if (
		...
		throw new Error('Could not get config.', response)
	}
